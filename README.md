...
**反在线解压**，就是要打破某些网盘对压缩文件的自动扫描。当你在某些网盘上传压缩包时，它们会在后台自动解压你的文件，扫描其中的内容——无论你是否知情。
...
- AODK 文件极小（通常仅几百字节），应**通过安全渠道（如端到端加密聊天、U 盘当面拷贝等）单独分享**，绝不随加密数据一起上传
- AODF 文件即使上传至任何云存储，服务方也无法识别、解压或扫描其内容，可**通过任意渠道（网盘、邮件、即时通讯等）自由分发**
